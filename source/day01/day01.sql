...
SELECT
    ename ||'쨩' 사원이름, sal "이전 급여", (sal*2) "인상 급여"
FROM
    emp;
--------------------------------------------------------------------------------
/*
...
--------     
SELECT
    ename 이름, sal 원급여, sal+10000 "보너스 적용 급여",
    sal*1.5 "인상 급여", sal*12 연봉
FROM
    emp;
--------     
SELECT
    e.ename "사원 이름", e.sal 급여, e.mgr "상사 번호",
...
--------    
SELECT
    ename "사원 이름", sal 급여, grade "급여 등급"
FROM
    emp, salgrade
WHERE
    sal BETWEEN losal AND hisal;    --오름차순 정렬인듯.
--------     
SELECT
    distinct deptno "부서 번호"
FROM
    emp;